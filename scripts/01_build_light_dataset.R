...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# remove keywords for which we don't find rank_group from 1 to 10 for
# every search engine
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
search_results_light <-
  ...
  mutate(n = sum(1:10 %in% rank_group)) %>%
  ungroup()  %>%
  group_by(keyword_id) %>%
  mutate(incomplete = any(n!=10)) %>%
  ungroup() %>%
  filter(!incomplete) %>%
  select(-n, -incomplete)

print(dim(search_results_light)) # [1] 6324619      11
