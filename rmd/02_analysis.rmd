...
We see below that on average 73% of top 10 results on yahoo can be also found in the top 10 of Bing,
making them the most similar pair.

Google is very dissimilar to other search engines, especially Yahoo, its shares only
28% of top 10 results wit the latter.
...
We see overall that top3 generally the higher the number of top results considered,
...
An exception is the Yahoo and Bing pair, which is the most similar when considering the full top 30.

We see that many of google's results are not found by its competitors (and inversely), 
in particular on average only 20% to 25% of Google's top 30 results are found in
...
#   geom_col() + 
#   theme(legend.position = "none", axis.text.x = element_text(angle = 30)) +
#   labs(title = "Similarity between top 20 results of search engine pairs",
#        x = NULL,
#        y = "Average similarity") +
#   scale_y_continuous(labels = scales::label_percent()) 
# 
...
#   geom_col() + 
#   theme(legend.position = "none", axis.text.x = element_text(angle = 30)) +
#   labs(title = "Similarity between top 30 results of search engine pairs",
#        x = NULL,
#        y = "Average similarity") +
#   scale_y_continuous(labels = scales::label_percent()) 
```
...
<!-- If the search engine contained Google's #1 result in its top 10 for all of their keywords their accuracy would be 100%. While if they did not, their accuracy would be 0%. -->

<!-- We show below the accuracy for each search engine using the top 10 as well as the top 20 and top 30 results. -->

<!-- For DuckDuckGo for example, we see that for 67% of keywords Google's -->
<!-- top result is included in DuckDuckGo's top 10, for 74% of keywords it's included in its top 30. -->
...
The following chart shows that there is significant variance in similarity depending on what category the keyword belongs too. Real Estate and Travel & Tourism are the categories where the top 10 between
Google and other search engines is the most similar, while Apparel is the category showing the most dissimilar top 10.
...
On average accuracy for length 1 we measure a similarity in the 24-27% range,
...
for smaller volume (generally smaller minima and higher maxima). It might be that for low volume searches, a high number of
...
* IMDB is less featured by Google than by competitors
