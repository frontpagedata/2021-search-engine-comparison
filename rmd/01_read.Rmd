...
```{r}
test_import <- read_delim(
            here::here(str_c("raw_data/download_from_gdrive/", "Google", "/", "0-5000.csv")),
            delim = "~",
            escape_double = F,
            escape_backslash = F,
            col_names = str_c("col_", 1:40),
            col_types = cols(.default = col_character())
)
...
```{r}
search_results %>% 
  filter(str_detect(keyword, "st louis"))
```

```{r}
search_results %>% 
  filter(search_engine == "Bing") %>% 
  arrange(desc(rank_absolute)) %>% 
  head(100)
  distinct(keyword, .keep_all = T)
```
...
```{r}
search_results %>% 
  filter(keyword == "financial loan companies")
```
...
```{r}
search_results %>% 
  filter(keyword == "prepaid customer service att")
```
